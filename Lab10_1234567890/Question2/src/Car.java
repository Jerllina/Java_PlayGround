...
public class Car {
	...
	public Car(String name, int numberOfDoors) throws BadCarException {
		...
		if(numberOfDoors < 1) {
			...
			// Create an arraylist for the doors of the car.
			// Java arraylists are objects so you have to use 'new'!
			doors = new ArrayList<Door>();
			// Create each door and add it at the end of the arraylist.
	...
	// length becomes size() and [i] becomes get(i, ...)
	public void openOneDoor(int doorNumber) throws BadDoorException {
		// Check that the door exists:
		if(1 <= doorNumber && doorNumber <= doors.size()) {
			// Array indexes go from 0 to doors.size() - 1 but doorNumber
			// is between 1 and doors.size() so we have to be careful to
			// subtract 1 from doorNumber before using it as an arraylist index
	...
	// length becomes size() and [i] becomes set(i, ...)
	...
	// We just add two new doors and we are done!  Very easy!
	...
	public static void testCar() {
		...
		// The error message inside the exception object changes too!
			...
			System.out.println(ex.getMessage().equals("Index: 7, Size: 7"));
		}
